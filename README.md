...
실행하면 메인 창에서 감지된 에이전트별 사용량과 제한 초기화 기준을 한눈에 확인할 수 있습니다.

![QuotaGlass 메인 창에서 에이전트별 사용량을 확인하는 화면](docs/full.png)
...
항상 위에 유지합니다. 같은 메뉴의 `그래프 세로로 배치`를 켜면 provider 그래프를
  위아래로 쌓아 표시할 수 있습니다. `위젯 표시 항목`의
`CPU/메모리 그래프 표시`를 켜면 시스템 리소스 그래프를 함께 표시하며, 기본값은
숨김입니다. `위젯 투명도`에서 0%, 25%, 50%, 75%를 선택하고
...
자유 이동 모드에서는 위젯이 작은 형태로 축약되어 다른 창 위에 표시됩니다.

위젯 오른쪽에는 시스템 전체 CPU와 물리 메모리 사용률을 합친 스파크라인이 표시됩니다.
처음 실행할 때 즉시 메모리를 측정하고, 이후 CPU와 메모리 이력을 5초마다 갱신합니다.
최근 24개 샘플(약 2분)을 그래프로 유지합니다.

![화면 위에 떠 있는 축약형 QuotaGlass 위젯](docs/floating.png)

고정 모드에서 위젯을 다른 모니터로 끌면 Windows의 `모든 디스플레이에 작업표시줄
표시` 옵션이 켜진 경우 해당 모니터의 작업표시줄로 이동하며, 옵션이 꺼져 있으면
메인 모니터의 작업표시줄에 남습니다.

작업표시줄에 고정된 위젯은 Windows 테마에 맞춰 밝은 테마와 어두운 테마로 표시됩니다.

| 밝은 테마 | 어두운 테마 |
| --- | --- |
| ![밝은 테마의 작업표시줄 위젯](docs/dock-1.png) | ![어두운 테마의 작업표시줄 위젯](docs/dock-2.png) |
